...
void addEmp(DList *list, Employee *emp)
{
    Node *node = newNode(emp);
    ...
    {
        node->prev = list->pLast;
        list->pLast = list->pLast->next = node;
    }
}
...
Node* searchName(DList *list, char *name)
{
    Node* current = list->pFirst;
    while(current != NULL)
    {
...
void display(DList *list, int id)
{
    displayEmployee(search(list, id)->data);
}

void deleteAll(DList *list)
{
    Node *temp;
...
void deleteNode(DList *list, Node *node)
{
    if (node != NULL)
    {
        if (node->prev == NULL)
        {
            list->pFirst = node->next;
            if (list->pFirst != NULL)
                list->pFirst->prev = NULL;
        }
        else if (node->next == NULL)
        {
            list->pLast = node->prev;
            list->pLast->next = NULL;
        }
        else if (list->pFirst == list->pLast)
        {
...
void deleteEmp(DList *list, int id)
{
    Node *node = search(list, id);
    deleteNode(list, node);
}

void deleteEmpName(DList *list, char *name)
{
...
char **lineEditor(int count, int *size, int *x, int *y, char *sRange, char *eRange, char **allowedChar)
{
    ...
    {
        line[i] = malloc((size[i] + 1) * sizeof(char));
    ...
    do
    {
        ...
        case Enter:
            ...
            case 0: // new
                ...
                tempNode = search(&emp, empPtr->ID);
                int found = tempNode != NULL;
            ...
            case 1: // display by ID
                ...
                scanf("%i", &ID);
                _flushall();
                displayEmployee(search(&emp, ID)->data);
                waitKey();
                break;
