...
namespace Exams
{
    ...
    {
        public AnswerList TrueAnswer { get; private set; }
        ...
        {
            if (correct == null || correct.Length > Choices.Count || correct.Length == 0)
            {
            ...
            {
                if (correct[c] < 0 || correct[c] >= Choices.Count)
                {
                    throw new ArgumentException("Wrong list of correct answers");
                }

                TrueAnswer.Add(Choices[correct[c]]);
            }
        }

        public override float GradeQuestion(AnswerList studentChoices)
        {
            if (studentChoices == null || studentChoices.Count > TrueAnswer.Count) return 0f;
            ...
            {
                found = false;
                for (int i = 0; i < studentChoices.Count && !found; i++)
                {
